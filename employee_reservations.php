...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_reservation'])) {
    ...
    if ($status == 'confirmed' && empty($table_number)) {
        ...
        header("Location: employee_reservations.php");
        exit();
    }
...
$get_reservation_stmt->close();
    ...
    // Check if the selected table is available for the reservation time
    if ($status == 'confirmed' && !empty($table_number)) {
    ...
    // Determine payment status based on new status
    ...
    if ($stmt->execute()) {
        ...
        // Send payment email only when status changes to confirmed
        if ($status == 'confirmed' && $reservation['status'] != 'confirmed') {
...
// Get all tables and group by capacity
$tables_by_capacity = [
    '10' => [],
    '6' => [],
    '2' => []
...
// Get reserved tables for each pending reservation
...
.status-no_show {
  ...
  border: 1px solid #E91E63;
        ...
        .tab-nav {
            display: flex;
            border-bottom: 1px solid rgb(0, 125, 4);
...
<div class="employee-sidebar">
    ...
    <ul class="employee-nav">
        ...
        <li><a href="employee_reservations.php" class="active"><i class="fas fa-calendar-alt"></i> Reservations</a></li>
...
<div class="employee-main">
    <div class="employee-header">
        ...
        <h1 class="employee-title">Reservations Management</h1>
